...
uint8_t address_tempsensor2[8] = { 0x28, 0xFF, 0x36, 0x3C, 0xA2, 0x16, 0x05, 0xCA  };   /* Direccion del sensor temperatura 2*/
...
void loop() 
{
  ...
  if(FlagSampling)                      /* Si el tiempo de muestreo es se activa se ejecuta el codigo principal*/
  {
    CounterSamples++;                   /* Toma muestra cada frecuencia de muestreo*/
...
bool positionSensor(void)
{
  ...
  next_sample = mpu6050Read(MPU_addr, false);                       /* Se realiza la lectura del sensor */
  sensor_values = convertRawToScaled(MPU_addr, next_sample,false);  /* Se escalan los valores leidos    */
  delay(1500);                                                      /* Retraso de 500 milisegundos      */

  if(sensor_values.AcZ >= 0)    /* Si el eje Z es positivo*/
    north = true;               /* el norte esta hacia el norte*/
  else
    north = false;              /* Sino el norte sera el sur*/
  return north;
}
...
/********************************************************************************************/
/*!
  \brief      Funcion que realiza el promedio de las 3 muestras y lo manda a la nube
  \param[in]  void
  \return     void
 */
void averageSensors(void)
{
  static float tmp1MoistureInside = 0;      /* Muestra 1 de humedad interna*/
  static float tmp2MoistureInside = 0;      /* Muestra 2 de humedad interna*/
  static float tmp3MoistureInside = 0;      /* Muestra 3 de humedad interna*/

  static float tmp1MoistureOutside = 0;     /* Muestra 1 de humedad externa*/
  static float tmp2MoistureOutside = 0;     /* Muestra 2 de humedad externa*/
  static float tmp3MoistureOutside = 0;     /* Muestra 3 de humedad externa*/

  static float tmp1TemperatureInside = 0;   /* Muestra 1 de temperatura interna*/
  static float tmp2TemperatureInside = 0;   /* Muestra 2 de temperatura interna*/
  static float tmp3TemperatureInside = 0;   /* Muestra 3 de temperatura interna*/

  static float tmp1TemperatureOutside = 0;  /* Muestra 1 de temperatura externa*/
  static float tmp2TemperatureOutside = 0;  /* Muestra 2 de temperatura externa*/
  static float tmp3TemperatureOutside = 0;  /* Muestra 3 de temperatura externa*/ 

  switch(CounterSamples)
  {
    case 0:
    break;
    case 1:   /* Asignacion de primera muestra de variables globales*/
      tmp1MoistureInside = MoistureInside;
      tmp1MoistureOutside = MoistureOutside;
      tmp1TemperatureInside = TemperatureInside;
      tmp1TemperatureOutside = TemperatureOutside;
    break;
    case 2:   /* Asignacion de segunda muestra de variables globales*/
      tmp2MoistureInside = MoistureInside;
      tmp2MoistureOutside = MoistureOutside;
      tmp2TemperatureInside = TemperatureInside;
      tmp2TemperatureOutside = TemperatureOutside;  
    break;
    case 3:   /* Asignacion de tercera muestra de variables globales*/
      tmp3MoistureInside = MoistureInside;
      tmp3MoistureOutside = MoistureOutside;
      tmp3TemperatureInside = TemperatureInside;
      tmp3TemperatureOutside = TemperatureOutside;
         
      /* Promedio de las 3 muestras para mandarlas a la nube*/
      MoistureInside = (tmp1MoistureInside + tmp2MoistureInside + tmp3MoistureInside)/3;
      MoistureOutside = (tmp1MoistureOutside + tmp2MoistureOutside + tmp3MoistureOutside)/3;
      TemperatureInside = (tmp1TemperatureInside + tmp2TemperatureInside + tmp3TemperatureInside)/3;
      TemperatureOutside = (tmp1TemperatureOutside + tmp2TemperatureOutside + tmp3TemperatureOutside)/3;

      /* Se limpian las variables temporales para iniciar de nuevo*/
      tmp1MoistureInside = 0;
      tmp2MoistureInside = 0;
      tmp3MoistureInside = 0;

      tmp1MoistureOutside = 0;
      tmp2MoistureOutside = 0;
      tmp3MoistureOutside = 0;

      tmp1TemperatureInside = 0;
      tmp2TemperatureInside = 0;
      tmp3TemperatureInside = 0;

      tmp1TemperatureOutside = 0;
      tmp2TemperatureOutside = 0;
      tmp3TemperatureOutside = 0;
      
      Serial.println();                                             /* Salto de linea de los datos                            */
      Serial.print("Average of Moisture Inside Sensor (%RH): ");    /* Impresion de humedad en el interior de la composta     */     
      Serial.println(MoistureInside);                               /* Dato de humedad interior de la composta                */

      Serial.print("Average of Temperature Inside Sensor (C): ");   /* Impresion de temperatura en el interior de la composta */    
      Serial.println(TemperatureInside);                            /* Dato de temperatura interior de la composta            */

      Serial.print("Average of Moisture Outside Sensor (%RH): ");   /* Impresion de humedad en el exterior de la composta     */   
      Serial.println(MoistureOutside);                              /* Dato de humedad exterior de la composta                */

      Serial.print("Average of Temperature Outside Sensor (C): ");  /* Impresion de temperatura en el exterior de la composta */   
      Serial.println(TemperatureOutside);                           /* Dato de temperatura exterior de la composta            */
      Serial.println();                                             /* Salto de linea de los datos                            */
    break;
    default:
